...
// BASIC SQL FUNCTIONS
			...
			throws SQLException
	{
		...
		conn = JdbcConnectionPooling.get_instance().get_connection();
		...
		// Close connection
		JdbcConnectionPooling.get_instance().close(conn);
	...
	/** delete tuple/s
	 * @param table	- the name of the table we want to delete from
	 * @param whereCl	- "WHERE column1 = ? AND column 2 = ? Or ..."
	...
	/** select tuple/s
	 * @param table	- the name of the table we want to delete from
	 * @param whereClause	- "WHERE column1 = ? AND column 2 = ? Or ..."
	 * @param values	- the objects to switch '?' in the where clause
	 * @throws SQLException
	 */
	protected static ResultSet select(String select, String table , String whereClause, Object... values) throws SQLException
	{
		...
		if (whereClause != null && whereClause != "" && !whereClause.contains("limit"))
			whereClause += " limit " + (new PropertyConfig().get_default_large_limit());
...
// Publics
	...
	public static Connection getConnection()
			throws SQLException
	{
		JdbcConnectionPooling jdbc_conn = JdbcConnectionPooling.get_instance();
	...
	public Statement getStatement()
			throws SQLException
	{
		JdbcConnectionPooling jdbc_con = JdbcConnectionPooling.get_instance();
